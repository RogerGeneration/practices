...
export default class extends Component {
  ...
  constructor (props) {
    ...
    this.state = {
      page:
        props.forms && props.forms.pagination ? props.forms.pagination.page : 0,
      query: props.query,
    };
  }
  ...
  onSort = field => {
    ...
    const currentSort =
      ...
      this.state.query.sort.length > 0 &&
      this.state.query.sort[0] === '-' ?
        ...
        <div>
          {access.submission.create ? (
            <span
              ...
              onClick={() => props.onAction(form, 'view')}
            >
          ...
          {access.submission.view ? (
            <span
              ...
              onClick={() => props.onAction(form, 'submission')}
            >
          ...
          {access.form.edit ? (
            <span
              className="btn btn-secondary btn-sm form-btn"
              onClick={() => props.onAction(form, 'edit')}
            >
          ...
          {access.form.delete ? (
            <span
              className="btn btn-danger btn-sm form-btn"
              onClick={() => props.onAction(form, 'delete')}
            >
  ...
  render () {
    ...
    const last =
      // eslint-disable-next-line radix
      skip + parseInt(limit) > pagination.total ?
        pagination.total :
        // eslint-disable-next-line radix
        skip + parseInt(limit);
